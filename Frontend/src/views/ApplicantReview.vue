          ...
          <div v-for="person in candidates" :key="person.id" class="candidate-card">
            ...
            <div class="candidate-right">
              ...
              <div class="action-buttons">
                <BaseButton variant="secondary" size="sm" @click="handleReject(person.id)">Pass</BaseButton>
                <BaseButton variant="primary" size="sm" @click="handleHire(person)">Hire</BaseButton>
              </div>
            </div>
...
.page-title { font-size: 1.8rem; font-weight: 800; margin: 0; color: #0f172a; }
...
.review-grid {
  display: grid;
  grid-template-columns: 300px 1fr; /* Fixed sidebar, fluid main */
  gap: 2rem;
  flex: 1;
...
.shift-item:hover { border-color: #cbd5e1; }
.shift-item.selected {
  border-color: #0f172a;
...
.shift-header { display: flex; justify-content: space-between; font-weight: 600; margin-bottom: 4px; }
.shift-role { color: #0f172a; }
.shift-date { color: #64748b; font-size: 0.9rem; }

.shift-meta { display: flex; justify-content: space-between; font-size: 0.8rem; color: #64748b; }
.applicant-count { color: #d97706; font-weight: 600; }
...
.candidates-header h3 { margin: 0; font-size: 1.25rem; font-weight: 400; }
.time-badge { background: #f1f5f9; padding: 4px 8px; border-radius: 4px; font-size: 0.85rem; font-weight: 600; }
...
.candidate-card:hover { transform: translateY(-2px); box-shadow: 0 4px 6px rgba(0,0,0,0.02); }

.candidate-left { display: flex; align-items: center; gap: 16px; }
...
.candidate-name { margin: 0 0 4px 0; font-size: 1.1rem; }
.trust-signals { font-size: 0.9rem; color: #64748b; display: flex; gap: 8px; }
.rating { color: #0f172a; font-weight: 600; }

.candidate-right { text-align: right; display: flex; flex-direction: column; gap: 10px; align-items: flex-end; }
.applied-time { font-size: 0.8rem; color: #94a3b8; }
.action-buttons { display: flex; gap: 8px; }

.empty-state {
  text-align: center; color: #94a3b8; margin-top: 3rem; font-style: italic;
}
</style>